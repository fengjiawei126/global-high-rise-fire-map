...
|---|---|---|---|---|
| Acute heat estimand for documented high-rise fires | Shi et al. 2025 analysed temperature–frequency responses for urban fire types in 2,847 cities; Yao et al. 2024 analysed long-run urban fire activity in China | Event-day versus same-weekday control days at the identical high-rise event coordinate and month, rather than a city-level temperature–frequency curve | ERA5-Land and NASA POWER products, six heat definitions, continuous lag families, two adjustment sets, core/recent/event-type exclusions, coastal-fallback exclusion and six leave-one-continent-out refits | Completed; ERA5-Land threshold OR 0.72 (0.24–2.18) does not support a positive association. Weather-adjusted continuous estimates are positive in both products but attenuate without weather covariates. Allowed: “tests distinct acute within-location estimands”. Prohibited: “proves heat or heatwaves increase high-rise fire risk” |
...
| Evidence architecture that can return a null, sign change or adjustment-sensitive result | Most urban-fire mapping studies optimize prediction or describe spatial risk | Product, definition, lag, adjustment, evidence-window and geographic checks are reported even when they weaken a simple heat narrative | ERA5-Land primary estimate is below one; NASA POWER primary estimate is above one; continuous estimates strengthen after simultaneous weather adjustment and attenuate in temperature-only models | Completed. Allowed: “quantifies product, exposure and estimand sensitivity”. Prohibited: “independent confirmation of a causal effect” |
...
The authenticated ERA5-Land extraction, independent NASA POWER processing, conditional-logistic estimation, restriction diagnostics and continuous-lag sensitivity are complete. The threshold-defined primary result is not a positive finding; the secondary continuous result is conditional and adjustment-sensitive. The remaining hard gate is author-confirmed metadata and public archival deposition. Any revision that turns either result into a causal null, a proven heatwave effect or an operational trigger fails this audit.
